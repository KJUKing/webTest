...
/*
	서블릿 동작 과정...
1. 사용자(클라이언트)가 URL을 클릭하면  HTTP요청(request)을 Serlvet Container로 전송(요청)한다.	
2. 컨테이너는 web.xml에 정의된 'url패턴'을 확인해서 어느 서블릿을 통해 처리해야 할 지를 검색한다.
   (로딩이 안된 경우에는 로딩을 한다. 처음 로딩시 init()메서드가 호출된다.)
   (서블릿 3.0이상에서는 @WebServlet 애노테이션으로 설정할 수 있다.)
...
public class ServletTest03 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<html><head><meta charset=utf-8></head>"
				+ "<body><h2 style='color:blue;'>goPost()메서드에서 처리한 결과입니다.</h2></body></html>");
	}
